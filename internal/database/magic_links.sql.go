...
func (q *Queries) CreateMagicLink(ctx context.Context, arg CreateMagicLinkParams) (MagicLink, error) {
	row := q.queryRow(ctx, q.createMagicLinkStmt, createMagicLink, arg.Token, arg.Email, arg.ExpiresAt)
	var i MagicLink
	err := row.Scan(
		&i.ID,
		&i.Token,
...
func (q *Queries) DeleteExpiredMagicLinks(ctx context.Context) error {
	_, err := q.exec(ctx, q.deleteExpiredMagicLinksStmt, deleteExpiredMagicLinks)
	return err
}
...
func (q *Queries) GetMagicLink(ctx context.Context, token string) (MagicLink, error) {
	row := q.queryRow(ctx, q.getMagicLinkStmt, getMagicLink, token)
	var i MagicLink
	err := row.Scan(
		&i.ID,
		&i.Token,
...
func (q *Queries) GetMagicLinkByEmail(ctx context.Context, email string) ([]MagicLink, error) {
	rows, err := q.query(ctx, q.getMagicLinkByEmailStmt, getMagicLinkByEmail, email)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) UseMagicLink(ctx context.Context, token string) error {
	_, err := q.exec(ctx, q.useMagicLinkStmt, useMagicLink, token)
	return err
}
